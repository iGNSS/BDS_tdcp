...
if (nargin < 2 || isempty(constellations)) %then use only GPS as default
    ...
    constellations.GLONASS = struct('numSat', 24, 'enabled', 0, 'indexes', [33:57], 'PRN', [1:24]);
    constellations.Galileo = struct('numSat', 30, 'enabled', 0, 'indexes', [58:88], 'PRN', [1:30]);
    constellations.BeiDou = struct('numSat', 47, 'enabled', 1, 'indexes', [89:135], 'PRN', [1:47]);
    constellations.QZSS = struct('numSat', 10, 'enabled', 0, 'indexes', [136:145], 'PRN', [1:10]);
    constellations.SBAS = struct('numSat', 10, 'enabled', 0, 'indexes', [146:155], 'PRN', [1:10]);
...
while (~feof(fid))
    ...
    if (~strcmp(sys_id, 'R'))
        ...
        Eph(23,i) = code_on_L2;
        Eph(24,i) = weekno;
        if (strcmp(sys_id, 'E') && weekno > 2500); weekno = weekno - 1024; end
        Eph(25,i) = L2flag;
        Eph(26,i) = svaccur;
        Eph(27,i) = svhealth;
        Eph(28,i) = tgd;
